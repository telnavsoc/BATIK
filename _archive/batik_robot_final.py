...
ASSETS_PATH = r"D:\eman\BATIK\assets"
COORD_FILE = "data_koordinat.json"
EVIDENCE_FOLDER = "EVIDENCE_LOGS"
DB_FILE = "pmdt_database_v2.db"
ANCHOR_X, ANCHOR_Y, ANCHOR_W, ANCHOR_H = 0, 0, 1024, 768

# Safety & Speed
pyautogui.FAILSAFE = True
pyautogui.PAUSE = 0.8 # Sedikit lebih cepat karena pakai validasi pintar
...
class HybridBatikRobot:
    ...
    def start_and_login(self):
        ...
        try:
            self.app = Application(backend="win32").connect(path=PMDT_PATH)
        except:
            self.app = Application(backend="win32").start(PMDT_PATH)
            time.sleep(10)
        
        self.handle_unexpected_popup()
        self.main_win = self.app.window(title_re=".*PMDT.*")
        
        if "No Connection" in self.main_win.window_text():
    ...
    def find_and_connect(self, station_name, image_file, expected_keyword):
        ...
        for attempt in range(3):
            ...
            if expected_keyword.upper() in title_now:
                 print(f"      ℹ️ Sudah terkoneksi ke {station_name}. Lanjut...")
                 return True

            print(f"   🔎 Percobaan #{attempt+1} menghubungkan...")
            img_path = os.path.join(ASSETS_PATH, image_file)
            ...
            if location and location.x < 300:
                ...
                time.sleep(0.5)         
                pyautogui.rightClick()  # Menu
                time.sleep(1.0) 
                
                print("      ⌨️ Memilih menu Connect (Down+Enter)...")
                ...
                # Validasi Keberhasilan Koneksi
                if expected_keyword.upper() in final_title:
                    print(f"      ✅ Sukses Masuk: {final_title}")
                    return True
                else:
                    print(f"      ❌ GAGAL: Judul '{final_title}' tidak sesuai.")
                    pyautogui.click(10, 200) # Reset fokus
            else:
                print(f"   ❌ Gambar {station_name} tidak ditemukan.")
                time.sleep(2)
        
        print(f"   🛑 Gagal total menghubungkan ke {station_name}. Skip.")
        return False

    def collect_data_and_disconnect(self, station_name, expected_keyword):
        # [GATEKEEPER PENTING] 
        # Cek apakah judul mengandung 'PC REMOTE' atau 'No Connection'
        # Jika YA, berarti disconnected -> JANGAN KLIK MONITOR! (Itu tombol Info)
        current_title = win32gui.GetWindowText(self.hwnd).upper()
        if "PC REMOTE" in current_title or "NO CONNECTION" in current_title:
            print(f"   ⛔ SAFETY STOP: Terdeteksi Disconnected. Membatalkan klik data untuk mencegah popup!")
            self.clean_disconnect() # Pastikan bersih
            return
        ...
        if x_mon and x_dat and x_copy:
            ...
            print("   ⏳ Menunggu data stabil (8 detik)...")
            time.sleep(8)
            ...
            if raw_text and len(raw_text) > 20:
                ...
                print(f"   ❌ GAGAL COPY: Clipboard kosong.")
        else:
            print("   ❌ Koordinat Data hilang.")

        # Panggil fungsi disconnect yang SUDAH DIPERBAIKI (No Blind Enter)
        self.clean_disconnect()

    def clean_disconnect(self):
        """Disconnection Sequence: System -> Disconnect -> Wait Title"""
        print("   🔌 Disconnecting System...")
        self.force_anchor_window()
        self.handle_unexpected_popup()
        ...
        if x_sys and y_sys and x_disc and y_disc:
            # Klik System
            print(f"      🖱️ Klik Menu System")
            pyautogui.click(x_sys, y_sys)
            time.sleep(0.5) 

            # Klik Disconnect
            print(f"      🖱️ Klik Menu Disconnect")
            pyautogui.click(x_disc, y_disc)
            
            # Hit and Run
            pyautogui.moveTo(5, 5) 
            
            # [OPTIMISASI WAKTU: CONFIRMATION BY TITLE]
            # Hapus delay 10 detik & Blind Enter.
            # Kita loop cek judul sampai berubah jadi "PC REMOTE"
            print("      ⏳ Menunggu status 'PC REMOTE'...")
            max_wait = 10
            ...
            while time.time() - start_wait < max_wait:
                current_title = win32gui.GetWindowText(self.hwnd).upper()
                if "PC REMOTE" in current_title or "NO CONNECTION" in current_title:
                    print("      ✅ Disconnect Terkonfirmasi.")
                    disconnected = True
                    break
                time.sleep(0.5)
            
            if not disconnected:
                print("      ⚠️ Waktu habis menunggu disconnect.")

        else:
            print("      ⚠️ Koordinat tidak lengkap!")
...
if __name__ == "__main__":
    print("🤖 BATIK ROBOT: V16 (OPTIMIZED DISCONNECT)")
